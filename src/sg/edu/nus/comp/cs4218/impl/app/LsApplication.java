...
public class LsApplication implements LsInterface {
	 ...
	 *            path to a file or the option that changes the behavior of the
	 *            application.
	 * @param stdin
	...
	/**
	 * Return the string listing the folder content of the specified folders. If no
	 * folder names are specified, list the content of the current folder.
	 * 
	 * @param isFoldersOnly
	 *            Boolean option to list only the folders.
	 * @param isRecursive
	 *            Boolean option to recursively list the folder contents (traversing
	...
	/**
	 * Scan args for ls options and file/folder paths and set it to options and
	 * paths respectively.
	 * 
	 * @param options
	 *            Array of Boolean indicating the respective options specified in
	 *            the args.
	 * @param paths
	 *            Vector of String containing the file/folder paths specified in the
	 *            args.
	 * @param args
	 ...
	 *             If an invalid option is specified
	 */
	private void setOptionsAndFiles(boolean[] options, Vector<String> paths, String... args) throws LsException {
		if (args == null) {
			return;
		}
	...
	/**
	 * Set existing files or folder in inputPaths into validPaths and return a
	 * String specifying the error files/folders that cannot be accessed. "." is
	 * added to validPaths if there is no inputPaths.
	 * 
	 * @param inputPaths
	 *            Array of String of file/folder path specified in args.
	 * @return paths Vector of String containing accessible files or folders.
	 * @throws LsException
	 *             If there is/are files/folders that cannot be access.
	 */
	private Vector<String> getValidPaths(String... inputPaths) throws LsException {
	...
	/**
	 * Sort the accessible files and folders in paths into files and folders vector
	 * respectively.
	 * 
	 * @param paths
	...
	private void sortFileAndDirectory(Vector<String> paths, Vector<String> files, Vector<String> folders) {
		for (int i = 0; i < paths.size(); i++) {
			File dir = LsExtension.getFileFromPath(paths.get(i), Environment.currentDirectory);
			if (dir.isDirectory()) {
	 ...
	 * @return String Formatted files for display.
	 */
	private String printFiles(Vector<String> files, String folder) {
		...
		for (int i = 0; i < files.size(); i++) {
			File dir = LsExtension.getFileFromPath(files.get(i), folder);
			if (!dir.isHidden()) {
	...
	/**
	 * Format folders for display. Include recursive folders and its contents if
	 * isRecursive is true.
	 * 
	 * @param folders
	 ...
	 * @return String Formatted folders, recursive folders and its' content for
	 *         display.
	 */
	...
	/**
	 * Format contents in the specified folder for display. Include recursive
	 * folders and its contents if isRecursive is true.
	 * 
	 * @param folder
	 *            String of the accessible folder.
	 * @param isRecursive
	 *            Boolean indicating recursive display of folders is specified.
	 * @return String Formatted folder contents and it's recursive folders and its'
	 *         content for display.
	 */
	private String printFilesInFolder(String folder, boolean isRecursive) {
		String outputStr = "";
		File dir = LsExtension.getFileFromPath(folder, Environment.currentDirectory);
