...
public final class DiffExtension {
	...
	/**
	 * Scans buffered reader and stores lines in hash set and vector for efficient
	 * computation
	 * 
	 * @param reader
	 *            buffered reader to retrieve lines from
	 * @param set
	 *            hashset to store lines in
	 * @param lines
	 *            vector to store lines in
	 * @param isNoBlank
	 *            flag to skip blank lines
	...
	/**
	 * checks if a given byte array contains any non-text characters
	 * 
	 * @param fileName
	 *            name of file containing byte array
	 * @param byteArray
	 *            byte array of file
	 * @throws DiffException
	 *             if byte array contains non-text characters
	 */
	public static void checkIfBinaryFile(String fileName, byte[] byteArray) throws DiffException {
		for (int i = 0; i < byteArray.length; i++) {
			int byteVal = byteArray[i];
			if (byteVal > 127 || byteVal == 0) {
				throw new DiffException(fileName + " is not a text file");
			}
		}
	}
	 ...
	 * 
	 * @param args
	 *            string array of arguments to extract from
	 * @param flags
	 *            boolean array to store flags
	 * @param hasFilesDirStream
	 *            boolean array to store if there are files, directories, or
	 *            inputstream
	 * @param files
