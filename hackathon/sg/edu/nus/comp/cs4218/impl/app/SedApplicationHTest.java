...
public class SedApplicationHTest {
    ...
    @Test
    public void testSedSpacePattern() throws AbstractApplicationException, ShellException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PrintStream pStream = new PrintStream(baos);
        System.setOut(pStream);
        shell.parseAndEvaluate("sed s/\\s/0/ two-lines.txt",
                System.out);
        assertEquals(
                "Hey,0good to know <you>!\n" +
                        "This0is a small file consists of {1+1+0} lines.\n" +
                        "/*0Hope this helps */ # no new line here",
                baos.toString());
    }

    @Test(expected = SedException.class)
    public void testSedNoFileInput() throws AbstractApplicationException, ShellException {
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PrintStream pStream = new PrintStream(baos);
        System.setOut(pStream);
        shell.parseAndEvaluate("sed s/v/a/", System.out);
    }
